...
import { generateDFSSteps, predefinedGraphs } from '@/utils/depthFirstSearch';
import { DFSStep, Graph } from '@/types/playground';
import { DFSPlaygroundControls } from './DFSPlaygroundControls';
...
function Stack({ stack }: { stack: string[] }) {
  return (
    <div className="space-y-2">
      <h4 className="text-sm font-semibold text-center">Pilha (Stack)</h4>
      <div className="flex flex-col-reverse gap-2 justify-center min-h-[200px] items-center bg-gray-50 dark:bg-gray-900 p-3 rounded-lg border-2 border-dashed border-gray-300">
        {stack.length === 0 ? (
          ...
          stack.map((nodeId, index) => (
            <motion.div
              ...
              className="w-12 h-12 rounded bg-purple-500 text-white flex items-center justify-center font-bold text-sm border-2 border-purple-600 shadow-md"
            >
              {nodeId}
            </motion.div>
          ))
      ...
      <p className="text-xs text-center text-gray-500">
        {stack.length > 0
          ? `Topo: ${stack[stack.length - 1]} | Base: ${stack[0]}`
...
function GraphVisualization({
  ...
  const getNodeColor = (nodeId: string) => {
    ...
    if (currentStep.stack.includes(nodeId)) {
      return '#a855f7'; // purple-500 - na pilha
    }
    if (nodeId === startNode) {
  ...
  const getEdgeColor = (from: string, to: string) => {
    ...
    if (currentStep.currentNode) {
      if (from === currentStep.currentNode || to === currentStep.currentNode) {
        ...
        // Se é uma conexão com um nó que está na pilha
        if (
          currentStep.stack.includes(from) ||
          currentStep.stack.includes(to)
        ) {
          return '#a855f7'; // purple-500 - adicionando à pilha
        }
      }
    }
    ...
    if (currentStep.currentNode) {
      if (from === currentStep.currentNode || to === currentStep.currentNode) {
        ...
        if (
          ...
          return 4; // Um pouco mais grossa ao empilhar
        }
      }
    }
...
export default function DFSPlayground() {
  ...
  const generateSteps = () => {
    const newSteps = generateDFSSteps(graph, startNode, targetNode);
    setSteps(newSteps);
    setCurrentStep(0);
    setIsPlaying(false);
  };
  ...
  const availableNodes = graph.nodes.map((node) => node.id);
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-center">
              Visualização da Busca em Profundidade (DFS)
            </CardTitle>
            <p className="text-center text-muted-foreground">
              Explore o grafo em profundidade até encontrar o nó alvo.
            </p>
          </CardHeader>
          <CardContent className="space-y-6">
            {/* Visualization */}
            <div className="grid md:grid-cols-2 gap-6">
              {/* Stack - Mobile: acima, Desktop: à direita */}
              <div className="md:order-2">
                <Stack stack={currentStepData.stack} />
              </div>
