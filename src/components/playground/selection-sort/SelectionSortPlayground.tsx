...
import { generateSelectionSortSteps } from '@/utils/selectionSort';
import { SelectionSortStep } from '@/types/playground';
import { SelectionSortPlaygroundControls } from './SelectionSortPlaygroundControls';
...
const PlaygroundContext = createContext<{
  currentStep: SelectionSortStep | null;
}>({
  currentStep: null
});
...
function ArrayBars({
  ...
  const getBarColor = (index: number) => {
    ...
    const { currentIndex, minIndex, comparingIndex, highlight, sortedIndices } =
      currentStep;
    ...
    // Elemento atual sendo processado
...
export default function SelectionSortPlayground() {
  ...
  const generateArray = (size: number) => {
    ...
    setArray(newArray);
    const newSteps = generateSelectionSortSteps(newArray);
    setSteps(newSteps);
    setCurrentStep(0);
    setIsPlaying(false);
  };
  ...
  return (
    <PlaygroundContext.Provider value={{ currentStep: currentStepData }}>
      <div className="w-full mx-auto space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-center text-muted-foreground">
                Acompanhe o processo de busca e seleção do menor elemento em
                tempo real.
              </p>
            </CardHeader>
        ...
        >
          <SelectionSortPlaygroundControls
