...
import { generateInsertionSortSteps } from '@/utils/insertionSort';
import { InsertionSortStep } from '@/types/playground';
import { InsertionSortPlaygroundControls } from './InsertionSortPlaygroundControls';
...
const PlaygroundContext = createContext<{
  currentStep: InsertionSortStep | null;
}>({
  currentStep: null
});
...
function ArrayBars({
  ...
  const getBarColor = (index: number) => {
    ...
    const { type, currentIndex, comparingIndex, highlight, sortedIndices } =
      currentStep;
    ...
    // Elemento sendo selecionado/inserido (key)
    if (type === 'select' && index === currentIndex) {
    ...
    // Elemento sendo inserido
    if (type === 'insert' && index === comparingIndex) {
...
export default function InsertionSortPlayground() {
  ...
  const generateArray = (size: number) => {
    ...
    setArray(newArray);
    const newSteps = generateInsertionSortSteps(newArray);
    setSteps(newSteps);
    setCurrentStep(0);
    setIsPlaying(false);
  };
  ...
  return (
    <PlaygroundContext.Provider value={{ currentStep: currentStepData }}>
      <div className="w-full mx-auto space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-center">
                Visualização do Insertion Sort
              </CardTitle>
              <p className="text-center text-muted-foreground">
                Acompanhe como cada elemento é inserido na posição correta da
                parte ordenada.
              </p>
            </CardHeader>
        ...
        >
          <InsertionSortPlaygroundControls
